...
parser.add_argument("--model", "-m", type=int, help='Model selection: 1 for ML only, 2 for ML + BLAST + MERCI', default=1)
...
os.system("python3 Standalone_Nfeature/RNA_Standalone/Nfeature_RNA.py -i" + fasta_loc + " -ft RDK -k 3 -o Standalone_Nfeature/RNA_Standalone/RDK3_results.csv")
os.system("python3 Standalone_Nfeature/RNA_Standalone/Nfeature_RNA.py -i"  + fasta_loc + " -ft RDK -k 4 -o Standalone_Nfeature/RNA_Standalone/RDK4_results.csv")
...
RDK3 = pd.read_csv("Standalone_Nfeature/RNA_Standalone/RDK3_results.csv", header=0)
RDK4 = pd.read_csv("Standalone_Nfeature/RNA_Standalone/RDK4_results.csv", header=0)
...
def pad_sequences(sequences, fixed_length=26):
    padded_sequences = []
...
def one_hot_encode_sequence(seq, fixed_length=26):
...
def generate_column_names(fixed_length=26):
    bases = 'ACGUX'
    column_names = []
...
def process_fasta_with_tfidf(fasta_file):
    ...
    feature_names_df = pd.read_csv('data/feature_names_tfidf.csv')
    ...
    vectorizer = TfidfVectorizer(analyzer='char', ngram_range=(1, 3), vocabulary=vocabulary)
...
combined_df = pd.concat([RDK34_df_n, ohe_df_n, tfidf_df], axis=1)
...
# Load the model
model_path = 'emirpred_et_model.pkl'
...
if model_choice == 1:
    ...
    db_path = 'blast_db/exosomal_db'
    output_path = 'blast_db/10e-2.txt'
    ...
    #evalue = 0.01
    dfb = pd.read_csv("blast_db/10e-2.txt", sep ="\t", header = None)
    dfb1 = dfb.iloc[:,0:3]
    ...
    dfb2.to_csv("blast_db/10e-2_results.txt", sep = "\t")
    dfb2 = pd.read_csv("blast_db/10e-2_results.txt", sep ="\t", header=0)
    ...
    os.system("perl MERCI/MERCI_motif_locator.pl -p" + fasta_loc + " -i MERCI/train_pos_motifs_g1.txt -o MERCI/pos_test_g1.txt")
    os.system("perl MERCI/MERCI_motif_locator.pl -p" + fasta_loc + " -i MERCI/train_pos_motifs.txt -o MERCI/pos_test.txt")

    import os

    # Define file paths
    file1 = "MERCI/pos_test_g1.txt"
    file2 = "MERCI/pos_test.txt"
    output_file = "MERCI/merci_ids.txt"
    ...
    merci_ids = pd.read_csv("MERCI/merci_ids.txt", sep ="\t", header=0)
    # Creating a set of query IDs for quick lookup
    ...
    prob_hybrid_df['total_pred'] = prob_df['ML_pred'] + prob_blast_df['blast_pred'] + prob_merci_df['merci_pred']
    ...
    hybrid_threshold = 0.52 if threshold == 0.5 else threshold  # Use 0.52 if default threshold is used for hybrid
